...
def plot_learning_curve(estimator, title, X, y, ylim=None, cv=None,
                        n_jobs=1, train_sizes=np.linspace(0.25, 1.0, 15)):
    """
...
plt.savefig("./weightedKNN/"+dataSet+" Sample Distribution of Training Data.png")
plt.clf()
plt.close()
...
if tuneParamaters:
    ...
    classifierParams = dict(n_neighbors=k,weights=['distance'])
    ...
    title = dataSet+' Learning Curves for Tuned Params (weightedKNN, Best K: '+ str(classifier.best_estimator_.n_neighbors)+' )'
    print "Best N Neighbors: ", classifier.best_estimator_.n_neighbors
    estimator = neighbors.KNeighborsClassifier(n_neighbors=classifier.best_estimator_.n_neighbors, weights='distance')
    plot_learning_curve(estimator, title, features_train, labels_train, cv=cv)
    plt.savefig("./weightedKNN/"+dataSet+ "Learning Curve .png")
    plt.clf()
    plt.close()

#############    SVM Learner    #############
#TODO: change clf below so that it uses the best params
clf = neighbors.KNeighborsClassifier(n_neighbors=classifier.best_estimator_.n_neighbors,weights='distance')
t0 = time()
...
plt.savefig("./weightedKNN/"+dataSet+" Sample Distribution of Testing Data.png")
plt.clf()
plt.close()
...
count = 0
for i in k:
    clf2 = neighbors.KNeighborsClassifier(n_neighbors=i, weights='distance')
...
plt.savefig("./weightedKNN/"+dataSet+" Score vs. K.png")
plt.clf()
plt.close()
