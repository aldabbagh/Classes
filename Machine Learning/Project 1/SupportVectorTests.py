...
def prepare_model(data,percentage):
...
training_data , testing_data =  prepare_model(data,0.7) #Splitting data into training and testing
...
if tuneParamaters:
    ...
    #gammas = np.logspace(-6, -1, 10).tolist()
    gammas = np.logspace(-15, 3, 19, base=2.0).tolist()
    Cs = np.logspace(-5, 15, 21, base=2.0).tolist()
    print len(gammas)
    #classifierParams = dict(gamma=gammas,C=[100000,100,1000,10000,10],kernel=['rbf','poly'])
    classifierParams = dict(gamma=gammas,C=Cs,kernel=['rbf'])
    classifierParams['gamma'].append('auto')
    print "Params: ", classifierParams
    classifier = GridSearchCV(estimator=svm.SVC(), cv=cv, param_grid=classifierParams)
    ...
    title = 'Learning Curves for Tuned Params (SVM,'+ str(classifier.best_estimator_.kernel)+' kernel,'+'$\gamma$'+'='+ str(classifier.best_estimator_.gamma) + 'C='+str(classifier.best_estimator_.C)
    estimator = svm.SVC(kernel=classifier.best_estimator_.kernel, gamma=classifier.best_estimator_.gamma,C=classifier.best_estimator_.C)
...
#############    SVM Learner    #############
#TODO: change clf below so that it uses the best kernel
clf = svm.SVC(kernel='rbf', C = classifier.best_estimator_.C, probability=True, gamma=classifier.best_estimator_.gamma)
t0 = time()


#building the classifier
training_data , testing_data =  prepare_model(data,0.7)

features_train = training_data[:,:-1] #selecting training data without labels
labels_train = training_data[:,-1] # assign training labels

features_test = testing_data[:,:-1] #Select testing data without labels
labels_test = testing_data[:,-1] #assign testing labels
...
plt.xlabel("Classes")
plt.ylabel("Count")
plt.title("Sample Distribution of Training Data")
...
plt.xlabel("Classes")
plt.ylabel("Count")
plt.title("Sample Distribution of Testing Data")
