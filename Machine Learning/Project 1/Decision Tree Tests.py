...
if tuneParamaters:
    ...
    nObjects = range(1,50,10)
    #classifierParams = dict(gamma=gammas,C=[100000,100,1000,10000,10],kernel=['rbf','linear','poly','sigmoid'])
    classifierParams = dict(max_depth=depths,criterion=['entropy'],min_samples_leaf =nObjects)
    print "Params: ", classifierParams
    classifier = GridSearchCV(estimator=tree.DecisionTreeClassifier(), cv=cv, param_grid=classifierParams)
    ...
    title = ' Learning Curves for Tuned Params (Decision Tree, Best Depth: '+ str(classifier.best_estimator_.max_depth)+' Criterion: '+str(classifier.best_estimator_.criterion)
    print "Best Depth: ", classifier.best_estimator_.max_depth
    estimator = tree.DecisionTreeClassifier(max_depth=classifier.best_estimator_.max_depth, criterion=classifier.best_estimator_.criterion, min_samples_leaf = classifier.best_estimator_.min_samples_leaf)
    plot_learning_curve(estimator,dataSet+title, features_train, labels_train, cv=cv)
    plt.savefig(outputFolder+dataSet+ " Learning Curve.png")
    plt.clf()
    plt.close()
#############    SVM Learner    #############
#TODO: change clf below so that it uses the best params
clf = tree.DecisionTreeClassifier(max_depth=classifier.best_estimator_.max_depth, criterion=classifier.best_estimator_.criterion, min_samples_leaf = classifier.best_estimator_.min_samples_leaf)
t0 = time()
...
print classifier.best_estimator_.min_samples_leaf
print classifier.best_estimator_.max_depth
...
#tree_train, tree_test = np.zeros(len(depths)), np.zeros(len(depths))
#for i in range(0,len(depths)):
#    clf2 = tree.DecisionTreeClassifier(max_depth=depths[i], criterion='entropy')
#    clf2 = clf2.fit(features_train, labels_train)
#    tree_train[i] = clf2.score(features_train,labels_train)
#    tree_test[i] = gridScores[i][1]

#plt.plot(depths,tree_test, linewidth=3, label = "Decision tree CVtest Score")
#plt.plot(depths,tree_train, linewidth=3, label = "Decision tree train Score")
#plt.legend()
#plt.ylim(0.5, 1.01)
#plt.xlabel("Max_depth")
#plt.ylabel("Score")
#plt.title(dataSet+" Score vs. Tree Depth")
#plt.savefig(outputFolder+dataSet+ " Score vs. Tree Depth.png")
#plt.clf()
#plt.close()
