...
dataSet = "Character" #could be either "Cancer" or "Character"

def plot_learning_curve(estimator, title, X, y, ylim=None, cv=None,
                        n_jobs=1, train_sizes=np.linspace(0.02, 1.0, 15)):
    """
...
def normalize(data,param = False):
    ...
    if not param:
        ...
        for i in range(len(data1[0])-1):
            tempMean = data1[:,i].mean()
            tempStd = data1[:,i].std()
            mean.append([tempMean,tempStd])
            data1[:,i] = (data1[:,i]-tempMean)/tempStd
        return data1, mean
    else:
        for i in range(len(data1[0])-1):
            data1[:,i] = (data1[:,i]-param[i][0])/param[i][1]
        return data1, param
...
plt.savefig("./KNN/"+dataSet+" Sample Distribution of Training Data.png")
plt.clf()
plt.close()
...
if tuneParamaters:
    ...
    classifierParams = dict(n_neighbors=k)
    print "Params: ", classifierParams
    classifier = GridSearchCV(estimator=neighbors.KNeighborsClassifier(), cv=cv, param_grid=classifierParams)
    ...
    title = dataSet+' Learning Curves for Tuned Params (KNN, Best K: '+ str(classifier.best_estimator_.n_neighbors)+' )'
    print "Best N Neighbors: ", classifier.best_estimator_.n_neighbors
    ...
    plt.savefig("./KNN/"+dataSet+ "Learning Curve .png")
    plt.clf()
    plt.close()

#############    SVM Learner    #############
#TODO: change clf below so that it uses the best params
clf = neighbors.KNeighborsClassifier(n_neighbors=classifier.best_estimator_.n_neighbors)
t0 = time()
...
plt.savefig("./KNN/"+dataSet+" Sample Distribution of Testing Data.png")
plt.clf()
plt.close()
...
tree_train, tree_test = np.zeros(len(k)), np.zeros(len(k))
count = 0
for i in k:
    ...
    #tree_test[count] = clf2.score(features_test,labels_test)
    #tree_test[i]=metrics.roc_auc_score(labels_test, clf2.predict_proba(features_test)[:,1])
    #tree_train[i]=metrics.roc_auc_score(labels_train, clf2.predict_proba(features_train)[:,1])
    count +=1
plt.plot(k,tree_test, linewidth=3, label = "Boosting CVtest Score")
plt.plot(k,tree_train, linewidth=3, label = "Boosting train Score")
plt.legend()
plt.ylim(0.5, 1.01)
plt.xlabel("K")
plt.ylabel("Score")
plt.title(dataSet+" Score vs. K")
plt.savefig("./KNN/"+dataSet+" Score vs. K.png")
plt.clf()
plt.close()
